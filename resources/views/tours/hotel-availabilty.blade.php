<x-layout.default>
    @section('title',$hotel->name. ' Availabilty')
    <div x-data="hotels">
            ...
            <template x-if="displayType === 'list'">
                <div class="table-responsive">
                    <table class="table-striped table-hover whitespace-nowrap">
                        ...
                        <tbody>
                            @foreach ($hotel->exceptions as $excep)
                            <tr>
                                <td>{{$i++}}</td>
                                <td >{{$excep->start_date}}</td>
                                <td >{{$excep->end_date}}</td>
                                <td  >{{$excep->new_price}}</td>
                                <td  >{{$hotel->price}}</td>
                                <td>
                                    <div class="flex gap-4 items-center ">
                                        <a href="{{route('hotels.availabilty.edit.form.show',$excep->id)}}" role="button" class="btn btn-sm btn-outline-primary">Edit</a>
