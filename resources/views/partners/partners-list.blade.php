<x-layout.default>
	...
	<div x-data="partners">
		<div class="flex items-center justify-between flex-wrap gap-4">
			<h2 class="text-xl">Partners</h2>
			<div class="flex sm:flex-row flex-col sm:items-center sm:gap-3 gap-4 w-full sm:w-auto">
				<div class="flex gap-3">
					@if (auth()->user()->role_id == 1)
						<div>
							<a href="{{route('partner.create')}}" role="button" class="btn btn-primary">
							...
							<tr>
								...
								<td>
									<form id="delete-form-{{ $partner->id }}"
										...
										@csrf
										@method('DELETE')
										<a href="#" role="button" class="btn btn-sm btn-outline-danger inline-block"
										   onclick="showAlert('{{ $partner->id }}')">Delete</a>
										<a href="{{route('partner.edit',$partner->id)}}" role="button"
										   class="btn btn-sm btn-outline-primary inline-block">Edit</a>
									</form>
	...
	<script>
		document.addEventListener("alpine:init", () => {
			Alpine.data("partners", () => ({
				...
				searchPartners() {
					this.filteredPartnersList = this.partnersList.filter(( d ) => d.name.toLowerCase()
						.includes(this.searchPartner.toLowerCase()));
				},
