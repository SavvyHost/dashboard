<x-layout.default>
    @section('title','Subfeatures')
    <div x-data="subfeatures">
        <div class="flex items-center justify-between flex-wrap gap-4">
            <h2 class="text-xl">Subfeatures</h2>
            <div class="flex sm:flex-row flex-col sm:items-center sm:gap-3 gap-4 w-full sm:w-auto">
                <div class="flex gap-3">
                    @if (auth()->user()->role_id == 1)
                    <div>
                        <a href="{{route('subfeature.create')}}" role="button" class="btn btn-primary" >
                                ...
                                <td>{{$subfeature->icon}}</td>
                                    <td>
                                        <form id="delete-form-{{ $subfeature->id }}" action="{{ route('subfeature.destroy', $subfeature->id) }}" method="POST" class="inline-block">
                                            @csrf
                                            @method('DELETE')
                                            <a href="#" role="button" class="btn btn-sm btn-outline-danger inline-block" onclick="showAlert('{{ $subfeature->id }}')">Delete</a>
                                            <a href="{{route('subfeature.edit',$subfeature->id)}}" role="button" class="btn btn-sm btn-outline-primary inline-block">Edit</a>
                                        </form>
    ...
    <script>
        document.addEventListener("alpine:init", () => {
            Alpine.data("subfeatures", () => ({
                ...
                filteredSubfeaturesList: [],
                searchSubfeature: '',
                subfeaturesList: {!! $subfeatures !!},

                init() {
                    this.searchSubfeatures();
                },

                searchSubfeatures() {
                    this.filteredSubfeaturesList = this.subfeaturesList.filter((d) => d.name.toLowerCase()
                        .includes(this.searchSubfeature.toLowerCase()));
                },
    ...
    <script>
        function showAlert(subfeatureId) {
            Swal.fire({
                title: 'Are you sure?',
                ...
                document.getElementById('delete-form-' + subfeatureId).submit();
                }
            });
}
    </script>
