                            ...
                            @endphp
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <div class="flex gap-4 items-center">
                                            <a href="{{ route('role.update', $role->id) }}" role="button" class="btn btn-sm btn-outline-primary">Edit</a>
                                            <a href="#" role="button" class="btn btn-sm btn-outline-danger" onclick="event.preventDefault(); document.getElementById('delete-form-{{ $role->id }}').submit();">Delete</a>
                                        </div> --}}
                                        <form id="delete-form-{{ $role->id }}" action="{{ route('role.destroy', $role->id) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
                                        ...
                                        <form id="delete-form-{{ $role->id }}" action="{{ route('user.role.update', $role->id) }}" method="POST">
                                            @csrf
                                            @method('PUT')
                                        </form>

                                        <div class="flex gap-4 items-center">
                                            <a href="{{ route('role.edit', $role->id) }}" role="button" class="btn btn-sm btn-outline-primary">Edit</a>
                                            <a href="#" role="button" class="btn btn-sm btn-outline-danger" onclick="event.preventDefault(); document.getElementById('delete-form-{{ $role->id }}').submit();">Delete</a>
                                        </div>
