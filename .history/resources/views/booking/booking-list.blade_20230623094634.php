                            ...
                            <tr>
                                <td>
                                    <div class="flex items-center w-max">
                                        <div >@if ($book->type == "room")
                                            Room
                                            @elseif($book->type == "tour")
                                            Tour
                                        @endif
                                    </div>
                                    </div>
                                ...
                                <td >{{$book->title}}</td>
                                {{-- <td >{{$book->customer_details->name}}</td> --}}

                                <td >{{$book->order_date}}</td>
                                <td >{{$book->customer_details->name}}</td>
                                <td >{{$book->total}}</td>
                                <td >@if ($book->payment_status == 0)
                                    Unpaid
                                    @elseif($book->payment_status == 1)
                                    Paid
                                ...
                                <td >@if ($book->payment_method == 0)
                                    Offline Payment
                                    @elseif($book->payment_method == 1)
                                    Offline Payment
                                    @elseif($book->payment_method == 2)
                                    Offline Payment
                                @endif</td>
                                <td>
                                    <div class="flex gap-4 items-center ">
                                        <a href="#" role="button" class="btn btn-sm btn-outline-success">Register Payment</a>
                                        <a href="#" role="button" class="btn btn-sm btn-outline-danger">Delete</a>
                                        <form action="{{ route('booking.confirm.store', $book->id) }}" method="POST">
                                            @csrf
                                            <button type="submit" class="btn btn-sm btn-outline-blue">Confirm</button>
                                        </form>
                                        <form action="{{ route('booking.reject.store', $book->id) }}" method="POST">
                                            @csrf
                                            <button type="submit" class="btn btn-sm btn-outline-blue">Reject</button>
                                        </form>
                                    </div>
                                </td>
