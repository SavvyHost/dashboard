    ...
    <div x-data="invoicePreview">
        ...
        <div class="panel">
            ...
            <div class="flex justify-between lg:flex-row flex-col gap-6 flex-wrap">
                <div class="flex-1">
                    <div class="space-y-1 text-white-dark">
                       <!-- Inside the booking.invoice.preview view -->
                        @if($booking->customer_details)
                        <h3>Customer:</h3>
                        <div class="text-black dark:text-white font-semibold">{{ $booking->customer_details->name }}</div>
                        <p>Username: {{ $booking->customer_details->username }}</p>
                        <p>Email: {{ $booking->customer_details->email }}</p>
                        <p>Phone: {{$booking->customer_details->phone }}</p>
                        <p>Country: {{ $booking->customer_details->country }}</p>
                        @else
            ...
            <div class="flex justify-between sm:flex-row flex-col gap-6">
                ...
                <div class="flex-1 sm:w-1/2">
                    <div class="xl:1/3 lg:w-2/5">
                        <div class="flex items-center w-full justify-between mb-2">
                            <div class="text-white-dark">Invoice ID:</div>
                            <div>{{$booking->invoice_id}}</div>
                        </div>
                        <div class="flex items-center w-full justify-between mb-2">
                    ...
                    <div class="xl:1/3 lg:w-2/5">
                        <div class="flex items-center w-full justify-between mb-2">
                            ...
                            <div class="whitespace-nowrap">{{$booking->status}}</div>
                        </div>
                        <div class="flex items-center w-full justify-between mb-2">
                            <div class="text-white-dark">Hotel:</div>
                            <div>{{$booking->room->hotels->name}}</div>
                        </div>
                        <div class="flex items-center w-full justify-between mb-2">
                            <div class="text-white-dark">Location:</div>
                            <div>{{$booking->room->hotels->locaton}}</div>
                        </div>
                        <div class="flex items-center w-full justify-between mb-2">
                    ...
                    <tbody>
                        {{-- <template x-for="item in items" :key="item.id"> --}}
                            <tr>
                                ...
                                <!-- Additional columns for room details -->
                                <td>
                                    <template x-for="roomType in {{$booking->room->types}}">
                                        <span x-text="roomType.name"></span>
                                    </template>
                                </td>                                <td x-text="">{{$booking->room->name}}</td>
                                <td class="ltr:text-right rtl:text-left" x-text="`$${$booking->room->price}`"></td>
                            </tr>
                        </template>
                    </tbody>
                </table>
