<x-layout.default>
    ...
    <div x-data="rooms">
        <div class="flex items-center justify-between flex-wrap gap-4">
            <h2 class="text-xl">Room Attributes</h2>
            <div>
                <ul class="flex space-x-2 rtl:space-x-reverse">
                    <li>
                        <a href="{{route('hotels.show')}}" class="text-primary hover:underline">Hotels</a>            </li>
                    <li class="before:content-['/'] ltr:before:mr-1 rtl:before:ml-1">
                        <a href="{{route('hotel.attr.show')}}" class="text-primary hover:underline">Attributes</a>
                    </li>
                    {{-- <li class="before:content-['/'] ltr:before:mr-1 rtl:before:ml-1">
                        <a href="{{route('hotel.attr.terms.show',$attr->id)}}" class="text-primary hover:underline">{{$attr->name}}</a>
                    </li> --}}
                </ul>
            </div>
            <div class="flex sm:flex-row flex-col sm:items-center sm:gap-3 gap-4 w-full sm:w-auto">
                <div class="flex gap-3">
                    ...
                    <div>
                        {{-- <a href="{{route('hotel.room.attr.form.show')}}" role="button" class="btn btn-primary" > --}}
                        <a href="{{url('/rooms/attribute/news')}}" role="button" class="btn btn-primary" >
                             {{-- <a href="{{route('hotel.room.attr.form.show')}}" class="text-primary hover:underline">Attributes</a> --}}
                             {{-- <a href="{{route('hotel.room.attr.form.show')}}" class="text-primary hover:underline">Attributes</a> --}}
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td>
                                    ...
                                    <td>
                                        <form id="delete-form-{{ $at->id }}" action="{{ route('hotel.room.attr.destroy', $at->id) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
                                            {{-- <button type="submit" class="btn btn-sm btn-outline-danger" >Delete</button> --}}
                                        </form>
                                        <div class="flex gap-4 items-center ">
                                            <a href="{{route('hotel.room.edit.attr.show',$at->id)}}" role="button" class="btn btn-sm btn-outline-primary">Edit</a>
                                            <a href="{{route('hotel.room.attr.terms.show',$at->id)}}" role="button" class="btn btn-sm btn-outline-success">Manage Terms</a>
                                            {{-- <a href="#" role="button" class="btn btn-sm btn-outline-danger" onclick="event.preventDefault(); document.getElementById('delete-form-{{ $at->id }}').submit();">Delete</a> --}}
                                            <a href="#" role="button" class="btn btn-sm btn-outline-danger" onclick="showAlert('{{ $at->id }}')">Delete</a>
    ...
    <script>
        function showAlert(atId) {
                Swal.fire({
                    ...
                    document.getElementById('delete-form-' + atId).submit();
                    }
                });
                }
