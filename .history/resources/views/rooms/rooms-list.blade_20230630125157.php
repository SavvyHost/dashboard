<x-layout.default>
    ...
    <div x-data="rooms">
        <div class="flex items-center justify-between flex-wrap gap-4">
            <h2 class="text-xl">Rooms</h2>
            <div class="flex sm:flex-row flex-col sm:items-center sm:gap-3 gap-4 w-full sm:w-auto">
                <div class="flex gap-3">
                    @if (auth()->user()->role_id == 1)
                    <div>
                        <a href="{{route('room.form.show')}}" role="button" class="btn btn-primary" >
                ...
                <div class="relative ">
                    <input type="text" placeholder="Search Rooms"
                        class="form-input py-2 ltr:pr-11 rtl:pl-11 peer" x-model="searchRoom"
                        @keyup="searchRooms" />
                    <div
                    ...
                    <table class="table-striped table-hover whitespace-nowrap">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td  ><a href="{{route('spec.room.availabilty.show',$room->id)}}" class="text-primary">Check Exceptions</a></td>
                                @if (auth()->user()->role_id == 1)
                                    <td  >{{$room->creation_date}}</td>
                                    <td>
                                            @csrf
                                            @method('DELETE')
                                        </form>
                                        <div class="flex gap-4 items-center ">
                                            ...
                                            <a href="#" role="button" class="btn btn-sm btn-outline-danger" onclick="showAlert('{{ $room->id }}')">Delete</a>
    ...
    <script>
        document.addEventListener("alpine:init", () => {
            Alpine.data("rooms", () => ({
                ...
                searchRooms() {
                    this.filteredRoomsList = this.roomsList.filter((d) => d.name.toLowerCase()
                        .includes(this.searchRoom.toLowerCase()));
                },
