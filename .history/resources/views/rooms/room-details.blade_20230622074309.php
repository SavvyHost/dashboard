<x-layout.default>
 ...
 --}}
                                    ...
                                    <div class="swiper-wrapper">
                                        @foreach ($imageArray as $image)
                                            <div class="swiper-slide">
                                                <img src="{{ asset('assets/hotel-photos/' . trim($image)) }}" class="w-full max-h-80 object-cover" alt="Hotel Image">
                                            </div>
                                        @endforeach
                                    </div>
    ...
    <script>
        ...
        // Get the room images from the server
        const roomImages = {!! json_encode($room->images) !!};
        const imageArray = roomImages ? roomImages.split(',') : [];

        // Set the images for the swiper
        const swiperImages = document.querySelectorAll("#slider1 .swiper-slide img");
        imageArray.forEach((image, index) => {
          if (swiperImages[index]) {
            swiperImages[index].src = `/assets/hotel-photos/${image.trim()}`;
          }
        });
      </script>
    <!-- end hightlight js -->
