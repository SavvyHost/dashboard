<x-layout.default>
    @section('title','Subscribers - Edit')
    ...
    <div>
        <ul class="flex space-x-2 rtl:space-x-reverse">
            <li>
                <a href="{{route('subscriber.show')}}" class="text-primary hover:underline">Subscribers</a>
            </li>
        ...
        <div class="pt-5">
            ...
            <div x-data="{ tab: 'home' }">
                ...
                <template x-if="tab === 'home'">
                    <div>@foreach ($subscribers as $subscriber)

                        <form method="POST" action="{{route('subscriber.update.form.save',$subscriber->id)}}"
                            ...
                            <div class="flex flex-col sm:flex-row">
                                <div class="flex-1 grid grid-cols-1 sm:grid-cols-2 gap-5">
                                    <div>
                                        ...
                                        <input id="name" type="text" name="name" value="{{$subscriber->name}}"
                                            class="form-input"   required/>
                                    </div>
                                    <div>
                                        <label for="email">Email</label>
                                        <input id="email" type="text"
                                            class="form-input" name="email" value="{{$subscriber->name}}" required />
                                    </div>
