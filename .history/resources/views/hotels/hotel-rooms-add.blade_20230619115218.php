...
foreach ($attrs as $attr) {
    ...
    foreach ($attr->terms as $term) {
        ...
        if (in_array($term->id, $request->input('terms', []))) {
            $totalPrice += $price;
        }
    }
}

// Calculate the price for the selected type
...
// Iterate over the $sups array and calculate the price for selected supplements
foreach ($sups as $sup) {
    $price = floatval($sup->price);
    if (in_array($sup->id, $request->input('sups', []))) {
        $totalPrice += $price;
    }
}
