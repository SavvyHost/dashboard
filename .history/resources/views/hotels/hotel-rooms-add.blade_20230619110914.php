    ...
    <script>
        function calculateTotal() {
            ...
            for (var i = 0; i < selectElements.length; i++) {
                ...
                // Remove deselected options from selectedOptions
                for (var j = selectedOptions.length - 1; j >= 0; j--) {
                    if (!options[selectedOptions[j]].selected) {
                        selectedOptions.splice(j, 1);
                    }
                }

                // Add newly selected options to selectedOptions
                for (var k = 0; k < options.length; k++) {
                    if (options[k].selected && !selectedOptions.includes(k)) {
                        selectedOptions.push(k);
                        var priceText = options[k].textContent.match(/\d+(\.\d+)?/);
                        if (priceText) {
