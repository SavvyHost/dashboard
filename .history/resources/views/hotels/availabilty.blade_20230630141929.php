<x-layout.default>
    @section('title','Hotels Availabilty')
    <div x-data="hotels">
        <div class="flex items-center justify-between flex-wrap gap-4">
            ...
            <div class="flex sm:flex-row flex-col sm:items-center sm:gap-3 gap-4 w-full sm:w-auto">
                <div class="flex gap-3">
                    ...
                    <div>
                        <a href="{{route('hotels.availabilty.form.show')}}" role="button" class="btn btn-primary" >
        ...
        <div class="mt-5 panel p-0 border-0 overflow-hidden">
            <template x-if="displayType === 'list'">
                <div class="table-responsive">
                    <table class="table-auto table-striped table-hover whitespace-nowrap">
                        <thead>
                            <tr >
                                <th>Name</th>
                                <th>Location</th>
                        ...
                        <tbody>
                            @foreach ($exceptions as $hotel)
                            <tr>
                                <td>
                                    <div class="flex items-center w-max">
                                        <div> <a href="{{route('hotels.availabilty.show',$hotel->id)}}" class="text-primary">{{$hotel->hotel_name}}</a> </div>
                                    </div>
                                </td>
                                <td >{{$hotel->location}}</td>
                                <td >{{$hotel->new_price}}</td>
                                <td >{{$hotel->start_date}}</td>
                                <td >{{$hotel->end_date}}</td>
                                <td>
                                    <form id="delete-form-{{ $hotel->id }}" action="{{ route('hotels.availabilty.destroy', $hotel->id) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        {{-- <button type="submit" class="btn btn-sm btn-outline-danger" >Delete</button> --}}
                                    </form>
                                    <div class="flex gap-4 items-center ">
                                        <a href="{{route('hotels.availabilty.form.show',$hotel->id)}}" role="button" class="btn btn-sm btn-outline-primary">Edit</a>
