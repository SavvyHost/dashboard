<x-layout.default>
    ...
    <div x-data="hotels">
        <div class="flex items-center justify-between flex-wrap gap-4">
            <h2 class="text-xl">Hotels</h2>
            <div class="flex sm:flex-row flex-col sm:items-center sm:gap-3 gap-4 w-full sm:w-auto">
                <div class="flex gap-3">
                    @if (auth()->user()->role_id == 1)
                    <div>
                        <a href="{{route('hotel.form.show')}}" role="button" class="btn btn-primary" >
                ...
                <div class="relative ">
                    <input type="text" placeholder="Search Hotels"
                        class="form-input py-2 ltr:pr-11 rtl:pl-11 peer" x-model="searchHotel"
                        @keyup="searchHotels" />
                    <div
                    ...
                    <table class="table-striped table-hover whitespace-nowrap">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                {{-- <td >{{$hotel->price}}</td> --}}
                                <td  ><a href="{{route('spec.hotel.availabilty.show',$hotel->id)}}" class="text-primary">Check Exceptions</a></td>
                                <td>
                                    @if (auth()->user()->role_id == 2)
                                             <a href="{{ route('hotel.rooms.show', $hotel->id) }}" role="button" class="btn btn-sm btn-outline-success">Show Rooms</a>
                                              @endif
                                </td>
                                @if (auth()->user()->role_id == 1)
                                    <td  >{{$hotel->creation_date}}</td>
                                    <td>
                                        <form id="delete-form-{{ $hotel->id }}" action="{{ route('hotel.destroy', $hotel->id) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
                                            {{-- <button type="submit" class="btn btn-sm btn-outline-danger" >Delete</button> --}}
                                        </form>
                                        <div class="flex gap-4 items-center ">
                                            <a href="{{route('hotel.edit.form.show',$hotel->id)}}" role="button" class="btn btn-sm btn-outline-primary">Edit</a>
                                            ...
                                            <a href="#" role="button" class="btn btn-sm btn-outline-danger" onclick="showAlert('{{ $hotel->id }}')">Delete</a>
                                            <a href="{{route('hotel.rooms.show',$hotel->id)}}" role="button" class="btn btn-sm btn-outline-success">Manage Rooms</a>
                                            <a href="{{route('hotel.details.show',$hotel->id)}}" role="button" class="btn btn-sm btn-outline-warning ">Details</a>
