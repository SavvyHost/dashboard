<x-layout.default>
    ...
    <div x-data="types">
        <div class="flex items-center justify-between flex-wrap gap-4">
            <div>
                <h2 class="text-xl">Cancellation Policy</h2>
                <ul class="flex space-x-2 rtl:space-x-reverse">
                    <li>
                        <a href="{{route('hotels.show')}}" class="text-primary hover:underline">Hotels</a>            </li>
                    <li class="before:content-['/'] ltr:before:mr-1 rtl:before:ml-1">
                        <a href="{{route('hotel.cancel.type.show')}}" class="text-primary hover:underline">Cancellation Policy</a>
                    </li>
                    <li class="before:content-['/'] ltr:before:mr-1 rtl:before:ml-1">
            ...
            <div class="flex sm:flex-row flex-col sm:items-center sm:gap-3 gap-4 w-full sm:w-auto">
                <div class="flex gap-3">
                    ...
                    <div>
                        <a href="{{route('hotel.cancel.type.form.show')}}" role="button" class="btn btn-primary" >
                            ...
                            <table class="table-striped table-hover whitespace-nowrap">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div class="flex items-center w-max">
                                                <div >{{$at->days_before_checkin}}</div>
                                            </div>
                                        </td>
                                        <td>
                                            <div class="flex items-center w-max">
                                                <div >{{$at->refund_percentage}}</div>
                                            </div>
                                        </td>
                                            <td>
                                                <form id="delete-form-{{ $at->id }}" action="{{ route('hotel.cancel.type.destroy', $at->id) }}" method="POST">
                                                    @csrf
                                                    @method('DELETE')
                                                    {{-- <button type="submit" class="btn btn-sm btn-outline-danger" >Delete</button> --}}
                                                </form>
                                                <div class="flex gap-4 items-center ">
                                                    <a href="{{route('hotel.cancel.edit.type.show',$at->id)}}" role="button" class="btn btn-sm btn-outline-primary">Edit</a>
                                                    {{-- <a href="{{route('hotel.room.type.destroy',$at->id)}}" role="button" class="btn btn-sm btn-outline-success">Manage Terms</a> --}}
