...
<script>
    $(document).ready(function() {
        // Track the location count
        var locationCount = 1;

        // Add Location button click event
        $("#addLocationBtn").click(function() {
            // Clone the first location fieldset
            var newLocation = $("#locationsContainer .row:first").clone();

            // Increment input name and ID attributes
            newLocation.find(".additional-location").each(function() {
                var name = $(this).attr("name");
                var newName = name.replace("[]", "[" + locationCount + "]");
                $(this).attr("name", newName);

                var id = $(this).attr("id");
                var newId = id + locationCount;
                $(this).attr("id", newId);
            });

            // Clear location input values
            newLocation.find(".additional-location").val("");
            newLocation.find(".additional-location").removeAttr("readonly");

            // Append the new location fieldset
