                                    ...
                                    <div class="row">
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                ...
                                                <input type="text" class="additional-location" name="latitudes[]" readonly>
                                                <input type="text" class="additional-location" name="longitudes[]" readonly>
                                            </div>
                                        </div>
                                    </div>
                                </div>

                                <button type="button" id="addLocationBtn" style="display: none;">Add Location</button>
                        {{-- <div class="flex sm:flex-row flex-col">
                            <label for="HotelLocation" class="mb-0 sm:w-1/4 sm:ltr:mr-2 rtl:ml-2">Location</label>
...
<script>
    $(document).ready(function() {
    // Show "Add Location" button when "Multi-Day Packages" option is selected
    $('input[name="category"]').change(function() {
        var selectedValue = $(this).val();
        if (selectedValue === "Multi-Day Packages") {
            $('#addLocationBtn').show();
        } else {
            $('#addLocationBtn').hide();
        }
    ...
    // Add new location fields when "Add Location" button is clicked
    $('#addLocationBtn').click(function() {
        var locationContainer = $('.locations-container');

        // Create a new map element
        var newMap = $('<div class="row"><div class="col-md-6"><div class="form-group"><label for="projectinput1">Location</label><input type="text" class="form-control additional-location" placeholder="Enter location" name="locations[]" required><input type="text" class="additional-location" name="latitudes[]" readonly><input type="text" class="additional-location" name="longitudes[]" readonly></div></div></div>');
        locationContainer.append(newMap);

        // Initialize the new map
        initMap(newMap.find('.additional-location:last-child'));
    });

    // Initialize the first map
    initMap('.additional-location');

    function initMap(inputElement) {
        var mapOptions = {
            // Set your map options here
        };

        var map = new google.maps.Map($(inputElement).prevAll('#map')[0], mapOptions);
        ...
        google.maps.event.addListener(marker, 'dragend', function() {
            ...
            $(inputElement).nextAll('input[name="latitudes[]"]').val(position.lat());
            $(inputElement).nextAll('input[name="longitudes[]"]').val(position.lng());
        });
        ...
        autocomplete.addListener('place_changed', function() {
            ...
            $(inputElement).nextAll('input[name="latitudes[]"]').val(place.geometry.location.lat());
            $(inputElement).nextAll('input[name="longitudes[]"]').val(place.geometry.location.lng());
        });
    }
});
</script>
