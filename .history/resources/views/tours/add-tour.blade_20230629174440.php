<x-layout.default>
                                ...
                                <div class="map-container">
                                    ...
                                    <div class="row">
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                <label for="projectinput1">Location</label>
                                                <input type="text" class="form-control additional-location" placeholder="Enter location" name="locations[]" required>
    ...
    <script>
        ...
        function initAutocomplete(mapElement, latitudeElement, longitudeElement) {
            ...
            function addMarker(location) {
                ...
                google.maps.event.addListener(marker, 'dragend', function() {
                    ...
                    latitudeElement.value = selectedLocation.lat();
                    longitudeElement.value = selectedLocation.lng();
                    geocodeLatLng(selectedLocation);
                });

                latitudeElement.value = location.lat();
                longitudeElement.value = location.lng();
            }
        ...
        function createMapContainer() {
            ...
            // Clone the first map container
            var clone = mapsContainer.firstElementChild.cloneNode(true);

            // Clear location input values and remove "readonly" attribute
            clone.querySelectorAll('.additional-location').forEach(function(element) {
                element.value = '';
                element.removeAttribute('readonly');
            });

            // Increment input names and IDs
            clone.querySelectorAll('.additional-location').forEach(function(element) {
                var name = element.getAttribute('name');
                var id = element.getAttribute('id');

                element.setAttribute('name', name + mapCount);
                element.setAttribute('id', id + mapCount);
            });

            // Append the cloned map container to the maps container
            mapsContainer.appendChild(clone);

            // Increment mapCount
            mapCount++;

            // Initialize the new map container
            var newMapElement = clone.querySelector('.map-container');
            var newLatitudeElement = clone.querySelector('.additional-location[name^="latitudes"]');
            var newLongitudeElement = clone.querySelector('.additional-location[name^="longitudes"]');
            initAutocomplete(newMapElement, newLatitudeElement, newLongitudeElement);
        }
        ...
        document.addEventListener("DOMContentLoaded", function(e) {
            ...
            // Initialize the first map container
            var firstMapElement = document.querySelector('.map-container');
            var firstLatitudeElement = document.querySelector('.additional-location[name="latitudes[]"]');
            var firstLongitudeElement = document.querySelector('.additional-location[name="longitudes[]"]');
            initAutocomplete(firstMapElement, firstLatitudeElement, firstLongitudeElement);

            // Add Location button click event listener
            addLocationBtn.addEventListener('click', createMapContainer);
        });
    </script>
