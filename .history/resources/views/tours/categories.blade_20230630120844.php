<x-layout.default>
    ...
    <div x-data="hotels">
        <div class="flex items-center justify-between flex-wrap gap-4">
            <h2 class="text-xl">Tour Categories</h2>
            <div class="flex sm:flex-row flex-col sm:items-center sm:gap-3 gap-4 w-full sm:w-auto">
                <div class="flex gap-3">
                    @if (auth()->user()->role_id == 1)
                    <div>
                        <a href="{{route('tour.category.form.show')}}" role="button" class="btn btn-primary" >
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td >{{$category->created_at}}</td>
                                    <td>
                                        <form id="delete-form-{{ $category->id }}" action="{{ route('tour.category.destroy', $category->id) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
                                            {{-- <button type="submit" class="btn btn-sm btn-outline-danger" >Delete</button> --}}
                                        </form>
                                        <div class="flex gap-4 items-center ">
                                            <a href="{{route('tour.category.edit.show',$category->id)}}" role="button" class="btn btn-sm btn-outline-primary">Edit</a>
                                            {{-- <a href="#" role="button" class="btn btn-sm btn-outline-danger" onclick="event.preventDefault(); document.getElementById('delete-form-{{ $category->id }}').submit();">Delete</a> --}}
                                            <a href="#" role="button" class="btn btn-sm btn-outline-danger" onclick="showAlert('{{ $category->id }}')">Delete</a>
                                        </div>
                                    </td>
