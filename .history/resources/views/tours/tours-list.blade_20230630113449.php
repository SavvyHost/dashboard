<x-layout.default>
    ...
    <div x-data="hotels">
        <div class="flex items-center justify-between flex-wrap gap-4">
            <h2 class="text-xl">Tours</h2>
            <div class="flex sm:flex-row flex-col sm:items-center sm:gap-3 gap-4 w-full sm:w-auto">
                <div class="flex gap-3">
                    @if (auth()->user()->role_id == 1)
                    <div>
                        <a href="{{route('tour.form.show')}}" role="button" class="btn btn-primary" >
                ...
                <div class="relative ">
                    <input type="text" placeholder="Search Tours By Name"
                    ...
                    <table class="table-striped table-hover whitespace-nowrap">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td >{{$tour->min_people}}</td>
                                <td >{{$tour->max_people}}</td>
                                @if (auth()->user()->role_id == 1)
                                    <td  >{{$tour->created_at}}</td>
                                    <td>
                                        <form id="delete-form-{{ $tour->id }}" action="{{ route('tour.destroy', $tour->id) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        {{-- <button type="submit" class="btn btn-sm btn-outline-danger" >Delete</button> --}}
                                    </form>
                                        <div class="flex gap-4 items-center ">
                                            <a href="{{route('tour.edit.form.show',$tour->id)}}" role="button" class="btn btn-sm btn-outline-primary">Edit</a>
                                            <a href="#" role="button" class="btn btn-sm btn-outline-danger" onclick="showAlert(event, '{{ $tour->id }}')">Delete</a>

                                            {{-- <a href="#" role="button" class="btn btn-sm btn-outline-danger" onclick="event.preventDefault(); document.getElementById('delete-form-{{ $tour->id }}').submit();">Delete</a> --}}
                                        </div>
                                    </td>
                                @endif
    ...
    <script>
function showAlert(event, tourId) {
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      document.getElementById('delete-form-' + tourId).submit();
    }
  });
}
    </script>
