<x-layout.default>
    ...
    <div x-data="hotels">
        <div class="flex items-center justify-between flex-wrap gap-4">
            <h2 class="text-xl">{{$attr->name}} Terms</h2>
            <div class="flex sm:flex-row flex-col sm:items-center sm:gap-3 gap-4 w-full sm:w-auto">
                <div class="flex gap-3">
                    @if (auth()->user()->role_id == 1)
                    <div>
                        <a href="{{route('tour.attr.terms.form.show',$attr->id)}}" role="button" class="btn btn-primary" >
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td>
                                    ...
                                    <td>
                                        <form id="delete-form-{{ $term->id }}" action="{{ route('tour.attr.term.destroy', $term->id) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
                                            {{-- <button type="submit" class="btn btn-sm btn-outline-danger" >Delete</button> --}}
                                        </form>
                                        <div class="flex gap-4 items-center ">
                                            <a href="{{route('tour.attr.terms.edit.show',$term->id)}}" role="button" class="btn btn-sm btn-outline-primary">Edit</a>
                                            ...
                                            <a href="#" role="button" class="btn btn-sm btn-outline-danger" onclick="showAlert(event, '{{ $term->id }}')">Delete</a>
    ...
    <script>
        function showAlert(event, termId) {
