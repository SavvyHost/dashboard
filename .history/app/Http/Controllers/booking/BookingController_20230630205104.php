...
class BookingController extends Controller
{
    ...
    public function my_invoice(Request $request, $bookingId)
    {
        $my_id = auth()->user()->id;
        $booking = Booking::where('customer', $my_id)
            ->with('room','customer_details')
            ->find($bookingId);
        ...
        return view('booking.invoice.preview', compact('booking'));
    }
    ...
    //     if ($selectedHotelId) {
    //         $selectedHotelRooms = Hotel::findOrFail($selectedHotelId)->rooms;
    //     }

    //     return view('booking.back-booking-list', compact('hotels', 'selectedHotelId', 'selectedHotelRooms'));
    // }
    ...
    public function back_booking(Request $request)
    {
        ...
        return view('booking.back-booking-list', [
            'hotels' => $hotels,
...
public function store(Request $request, $id)
{
    ...
    $booking->invoice_id = $this->generateInvoiceId();
        ...
        $currencyId = $this->getCurrencyIdFromType($request->input('type'), $id);
    if ($currencyId === null) {
        return redirect()->back()->with('error', 'Booking details not found');
    }
...
private function getCurrencyIdFromType($type, $id)
{
    if ($type === 'tour') {
        $tour = Tour::find($id);
        return $tour ? $tour->currency_id : null;
    } elseif ($type === 'room') {
        $room = Room::find($id);
        return $room ? $room->currency : null;
    }
