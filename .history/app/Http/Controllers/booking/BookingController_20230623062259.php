...
public function store(Request $request, $id)
{
    ...
    $title = $this->getTitleFromType($type, $id);
...
private function getTypeFromId($id)
{
    // Assuming 'tours' and 'rooms' are the table names for tours and rooms respectively
    if (Tour::where('id', $id)->exists()) {
        return 'tour';
    } elseif (Room::where('id', $id)->exists()) {
        return 'room';
    }
